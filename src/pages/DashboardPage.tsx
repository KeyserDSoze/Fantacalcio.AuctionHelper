...
export function DashboardPage({ data, onUndoPurchase }: { data: AppSnapshot; onUndoPurchase: (purchase: Purchase) => Promise<void> }) {
  ...
  const pendingPrices = data.purchases.filter((purchase) => purchase.pricePending).length;
  ...
  return <div className="space-y-4 sm:space-y-6">
    ...
    {pendingPrices > 0 && <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-600"><strong>{pendingPrices} {pendingPrices === 1 ? "prezzo da completare" : "prezzi da completare"}.</strong> I giocatori sono già assegnati, ma quei costi non sono ancora sottratti dai budget.</div>}
