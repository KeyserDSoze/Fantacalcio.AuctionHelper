...
import { Heart, Search, Upload, XCircle } from "lucide-react";
...
export function PlayersPage({ data, onPlayer, onCatalog }: { data: AppSnapshot; onPlayer: (p: Player) => Promise<void>; onCatalog: (players: Player[], clubs: AppSnapshot["clubs"]) => Promise<void> }) {
  ...
  const importFile = async (file?: File) => {
    if (!file) return;
    try { const parsed = await parseFantamasterFile(file, data.players, data.clubs); await onCatalog(parsed.players, parsed.clubs); setMessage(`${parsed.players.length} giocatori importati. Tier e preferenze esistenti preservati quando possibile.`); }
    catch (error) { setMessage(error instanceof Error ? error.message : "Errore durante l'import."); }
  };

  return <div className="space-y-6">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><h1 className="text-3xl font-bold">Giocatori</h1><p className="mt-1 text-muted-foreground">Import FantaMaster, preferenze, tier personali e scelta pianificata.</p></div><div><input ref={fileRef} type="file" accept=".xlsx,.xls" className="hidden" onChange={(e) => void importFile(e.target.files?.[0])} /><Button onClick={() => fileRef.current?.click()}><Upload className="h-4 w-4" />Importa Excel</Button></div></div>
