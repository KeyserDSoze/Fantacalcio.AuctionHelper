...
export function DataPage({ onExport, onImport }: { onExport: () => Promise<AppSnapshot>; onImport: (s: AppSnapshot) => Promise<void> }) {
  ...
  return <div className="space-y-6"><div><h1 className="text-3xl font-bold">Dati & backup</h1><p className="mt-1 text-muted-foreground">IndexedDB è locale al browser: esporta un backup prima dell'asta e nei momenti importanti.</p></div>{message && <div className="rounded-lg border border-primary/30 bg-primary/10 p-3 text-sm text-primary">{message}</div>}<div className="grid gap-4 md:grid-cols-2"><Card><CardHeader><CardTitle>Esporta snapshot</CardTitle></CardHeader><CardContent><p className="mb-4 text-sm text-muted-foreground">Giocatori, tier, preferenze, squadre, acquisti e fase corrente.</p><Button onClick={() => void download()}><Download className="h-4 w-4" />Esporta JSON</Button></CardContent></Card><Card><CardHeader><CardTitle>Ripristina snapshot</CardTitle></CardHeader><CardContent><p className="mb-4 text-sm text-muted-foreground">Sostituisce i dati locali con quelli contenuti nel file.</p><input ref={ref} className="hidden" type="file" accept="application/json,.json" onChange={(e) => void importJson(e.target.files?.[0])} /><Button variant="outline" onClick={() => ref.current?.click()}><Upload className="h-4 w-4" />Importa JSON</Button></CardContent></Card></div></div>;
}
